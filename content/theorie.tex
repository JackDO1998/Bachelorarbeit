...
Anordnung von Magneten beschreiben die durch fliegenden Teilchen eine sinusförmige Bahn. Die Amplitude des 
...
der modifizierten Besselfunktion zweiter Gattung zur Ordnung $5/3$. Die Spektralfunktionerfüllt die natürliche Normierungsbedingung $\int_{0}^{\infty}S(x)dx = 1$. \cite{Wille}
Um die Besselfunktion integrieren zu können wird diese mit Airy-Funktionen $Ai(z)$ und $Ai'(z)$ ausgedrückt. Dazu finden wir bei \cite{DLMF} die Zusammenhänge:
\begin{equation*}
    K_{\pm 1/3}(u) = \pi \sqrt{\frac{3}{z}} Ai\left(z\right)
\end{equation*}
\begin{equation*}
    K_{\pm 2/3}(u) = -\pi \sqrt{\frac{3}{z}} Ai'\left(z\right)
\end{equation*}
    mit 
\begin{equation*}    
    z = \left(\frac{3u}{2}\right)^{2/3}
\end{equation*}
sowie die Rekursion:
\begin{equation*}
    K_{\nu+1}(u) = K_{\nu-1}(u) + \frac{2\nu}{u}K_{\nu}(u)
\end{equation*}
so das sich leicht eine geschlossene Form für $K_{5/3}(u)$ finden lässt. 
\begin{equation*}
    K_{5/3}(u) = K_{-1/3}(u) + \frac{10}{3u}K_{2/3}(u) = \pi \sqrt{\frac{3}{z}} Ai\left(z\right) - \frac{10\pi}{3u}\sqrt{\frac{3}{z}} Ai'\left(z\right)
\end{equation*}
Diese kann nun leich mithilfe von Matlab integriert werden.
